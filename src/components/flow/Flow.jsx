...
function Flow() {
  ...
  // Extract caseId from route params or use default for now
  // This is prepared for dynamic caseId routing
  // When routing is updated to include caseId, this will automatically work
  const caseId = params.caseId || params.id || 1; // Fallback to 1 for backward compatibility
  const [networkFlowData, setNetworkFlowData] =
    useRecoilState(networkFlowDataAtom);
  ...
  const { mutate, isLoading: isSaving } = useFlowDiagram();

  // Use react-query for fetching flow data
  const {
    nodes: fetchedNodes,
    edges: fetchedEdges,
    diagramId,
  ...
  // Template functionality - using both local state and server operations
  const { saveTemplate: saveTemplateLocal } = useTemplateManager();
  const {
    saveTemplate: saveTemplateServer,
    isSavingTemplate,
    saveTemplateError
  ...
  const [selectedEdgeType, setSelectedEdgeType] =
    useRecoilState(selectedEdgeTypeAtom);
  ...
  // Set nodes and edges from processed data when loaded
  useEffect(() => {
    if (fetchedNodes.length > 0 && !isLoading) {
      ...
      // Auto-zoom and fit view after data is loaded
      setTimeout(() => {
        zoomTo(0.5);
  ...
  }, [fetchedNodes, fetchedEdges, isLoading, error, saved, setNodes, setEdges, setNetworkFlowData, fitView, zoomTo]);
    ...
    if (!isDeveloperMode) {
      ...
      if (hasTextBoxOverlap) {
        ...
        for (const n of nodeLookup.values()) {
          ...
          const contains =
            clickedNodeCenterX >= left &&
            clickedNodeCenterX <= right &&
            clickedNodeCenterY >= top &&
            clickedNodeCenterY <= bottom;
          if (!contains) continue;
          ...
          const overlapRight = Math.min(
            right,
            clickedNodePos.x + clickedNodeWidth
          );
          const overlapBottom = Math.min(
            bottom,
            clickedNodePos.y + clickedNodeHeight
          );
          const overlapArea =
  ...
  const handleSaveTemplate = useCallback(() => {
    ...
    try {
      console.log(
        "Selected nodes and edges for template:",
        selectedNodes,
        allEdges
      );
      ...
      // Save to server
      saveTemplateServer(templateData, {
        onSuccess: () => {
          // Also save locally for immediate UI updates
          saveTemplateLocal(name.trim(), selectedNodes, allEdges);
  ...
  }, [selNodes, selEdges, saveTemplateServer, saveTemplateLocal]);
  ...
  return (
    ...
    >
      ...
      {isDeveloperMode && showSaveTemplate && (
        ...
        >
          {isSavingTemplate ? "Saving..." : `Save as Template (${selNodes.length} node${selNodes.length !== 1 ? "s" : ""}, ${selEdges.length} edge${selEdges.length !== 1 ? "s" : ""})`}
        </button>
      )}
      <>
        <ReactFlow
