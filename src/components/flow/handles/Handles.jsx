...
const Handles = ({ height, width, numTargetHandlesLeft, numTargetHandlesTop, numSourceHandlesRight, numSourceHandlesBottom, key }) => {
  ...
  return (
    <>
      ...
      {Array.from({ length: numTargetHandlesLeft }).map((_, idx) => {
        ...
        return <Handle key={`target-handle-left-${key}`} type="target" position={Position.Left} id={`target-handle-left-${idx}`} style={handleStyle} />;
      })}
      ...
      {Array.from({ length: numTargetHandlesTop }).map((_, idx) => {
        ...
        return <Handle key={`target-handle-top-${key}`} type="target" position={Position.Top} id={`target-handle-top-${idx}`} style={handleStyle} />;
      })}
      ...
      {Array.from({ length: numSourceHandlesRight }).map((_, idx) => {
        ...
        return <Handle key={`source-handle-right-${key}`} type="source" position={Position.Right} id={`source-handle-right-${idx}`} style={handleStyle} />;
      })}
      ...
      {Array.from({ length: numSourceHandlesBottom }).map((_, idx) => {
        ...
        return <Handle key={`source-handle-bottom-${key}`} type="source" position={Position.Bottom} id={`source-handle-bottom-${idx}`} style={handleStyle} />;
      })}
    </>
  );
};
